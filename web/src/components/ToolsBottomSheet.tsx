...
interface ToolsBottomSheetProps {
  ...
  onAwsDocumentationChange: (value: boolean) => void;
  webSearch: boolean;
...
function ToolsBottomSheet({
  ...
  const tools = [
    ...
    },
    {
      ...
      checked: awsDocumentation,
      onChange: onAwsDocumentationChange,
      icon: (
        <svg
          className="h-5 w-5"
    ...
    },
    {
      ...
      onChange: onCodeInterpreterChange,
      icon: (
        <svg
          className="h-5 w-5"
          fill="none"
      ...
      <div
        ...
        <div className="max-h-96 overflow-y-auto">
          {tools.map((tool) => (
            <label
              key={tool.id}
              className="flex w-full cursor-pointer items-center px-4 py-2 transition-colors duration-200 hover:bg-gray-50 dark:hover:bg-gray-700">
              <div className="flex items-center">
                ...
                {/* Icon */}
                <div className="ml-3 flex h-8 w-8 items-center justify-center text-gray-600 dark:text-gray-400">
                  {tool.icon}
                </div>
